...
document.querySelector('#input__name').addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
        show();
    }
});
async function show() {
    ...
    drinkName = document.querySelector('#input__name').value;
    if(drinkName == '') document.querySelector('#output').innerHTML = '';
    else {
        await fetch(`https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${drinkName}`)
